...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	// instead of being sane we are gonna be a little sane
	// we will use our very own log for raft. We will simply wrap it up to satisft an interface
	...
	// ussually this is not needed. But we will change this so that our tests are faster
...
func (l *DistributedLog) Read(offset uint64) (*api.Record, error) {
	// may not be super up to date as it does not go through raft
...
func (l *DistributedLog) Join(id, addr string) error {
	...
	for _, srv := range configFuture.Configuration().Servers {
		if srv.ID == serverID || srv.Address == serverAddr {
			...
			// i.e. if there is like some discrprencies of id and addr
...
func (s *StreamLayer) Dial(addr raft.ServerAddress, timeout time.Duration) (net.Conn, error) {
	...
	_, err = conn.Write([]byte{byte(RaftRCP)})
	if err != nil {
		return nil, err
	}
...
func (s *StreamLayer) Accept() (net.Conn, error) {
	...
	if !bytes.Equal([]byte{byte(RaftRCP)}, b) {
