...
func (c *cli) setupConfig(cmd *cobra.Command, args []string) error {
	...
	if err = viper.ReadInConfig(); err != nil {
		// if the error is something other than config file not found then return an error
	...
	c.cfg.RCPPort = viper.GetInt("rpc-port")
	c.cfg.StartJoindAddrs = viper.GetStringSlice("start-join-addrs")
...
func (c *cli) run(cmd *cobra.Command, args []string) error {
	var err error
	marsh, _ := json.MarshalIndent(c.cfg.Config, "", "	")
	fmt.Println(string(marsh))

	agent, err := agent.New(c.cfg.Config)
	fmt.Println(c.cfg.Config)
	if err != nil {
		// debug.PrintStack()
		return err
	}
